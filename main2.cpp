...
//    status=SQLSetConnectAttr(dbc, SQL_ATTR_LOGIN_TIMEOUT, OTL_RCAST(void*,OTL_SCAST(size_t,timeout)), 0);
    ...
    ret = SQLPrepare( stmt, (SQLCHAR*)"select id, name, test from tuto;", SQL_NTS);
    if(ret != SQL_SUCCESS){
    ...
    ret = SQLBindCol(stmt, 1, SQL_C_LONG, &sId, 0, &sId_len);
    ret = SQLBindCol(stmt, 2, SQL_C_CHAR, sName, 129, &sName_len);
    ret = SQLBindCol(stmt, 3, SQL_C_CHAR, sTest, 129, &sTest_len);
    ...
    std::cout << "select columns : " << now_num_of_columns << std::endl;

    for(int i = 0 ; i < now_num_of_columns ; ++i){
        ret = SQLFetch(stmt);
        if(ret == SQL_NO_DATA){
        ...
        std::cout << "rSQLBindCol ret : " <<  ret << ", len : " << sId << std::endl;
        std::cout << "rSQLBindCol ret : " <<  ret << ", sName : " << sName << std::endl;
        std::cout << "rSQLBindCol ret : " <<  ret << ", sTest : " << sTest << std::endl;
